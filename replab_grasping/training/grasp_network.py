...
class GraspDetectionNet(nn.Module):
    ...
    def __init__(self, filters=64, fc_size=64):
        super(GraspDetectionNet, self).__init__()
        self.filters = filters
        self.fc_size = fc_size

        self.rgb_features = nn.Sequential(
            nn.Conv2d(3, filters, kernel_size=5, padding=2),
        ...
        self.depth_features = nn.Sequential(
            nn.Conv2d(1, filters, kernel_size=5, padding=2),
            nn.BatchNorm2d(filters),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=3),
            nn.Conv2d(filters, filters, kernel_size=5, padding=2),
    ...
    def forward(self, x):
        ...
        img = torch.cat([rgb, d], dim=1)

        grasp = torch.reshape(grasp, (grasp.size(0), 1))
        grasp = self.grasp_features(grasp)
        grasp = grasp.reshape(grasp.size(0), self.filters, 1, 1)
        grasp = grasp.repeat(1, 2, 21, 21)
        z = img + grasp
        z = self.conv_features(z)
        z = z.view(z.size(0), 4 * self.filters)
        z = self.classifier(z)
        return z
